...
const cards=rows.map((line,index)=>{const [,name,s,z,w,religion,rawNation]=line.split("\t");const nation=rawNation==="E"?"Et":rawNation;const spreadsheetRow=index+2;const id=`NK-ROW-${String(spreadsheetRow).padStart(3,"0")}`;const assets=assetsByRow.get(id)??[];const reconciledCopies=assets.reduce((n,a)=>n+a.copyCount,0);const availableInPrototype=assets.length>0;return{id,name,strength:Number(s),zeal:Number(z),wealth:Number(w),religion,nation,factionId:factionFor(religion,nation),type:"unknown",assets,deckCopies:availableInPrototype?reconciledCopies:0,availableInPrototype,source:{spreadsheetRow}}});
const invalid=cards.filter(c=>!c.factionId||![c.strength,c.zeal,c.wealth].every(Number.isFinite));
if(invalid.length)throw new Error(`Invalid canonical rows: ${invalid.map(c=>c.id).join(", ")}`);
const output={schemaVersion:1,source:"NK DB.xls",statisticsAuthority:"spreadsheet",cards};
