...
function MultiplayerBoard({ room, busy, error, onAct, onLeave }: { room: Room; busy: boolean; error: string; onAct: (body: object) => void; onLeave: () => void }) {
  ...
  return <main className="gamePage multiplayerGame"><header className="gameHeader"><div><EparchCrownMark className="miniMark" /><b>Nubian Kings</b><small>Beginner multiplayer · Round {state.round} · Room {room.code}</small></div><div className="toolbar"><button className="iconButton" onClick={() => navigator.clipboard?.writeText(room.code)}>Copy Room Code</button><button className="iconButton" onClick={onLeave}>Leave View</button></div></header>{room.review ? <MultiplayerReviewView room={room} /> : <div className="board"><div className="opponentBoard multiplayerPlayers">{state.players.map((player) => <PublicPlayerArea key={player.id} room={room} state={state} player={player} />)}</div></div>}{room.review ? <section className="chooser ready multiplayerContinue">{room.acknowledged ? <p>Waiting for the other players to continue…</p> : <button disabled={busy} onClick={() => onAct({ action: "acknowledge" })}>Continue</button>}</section> : room.status === "active" ? <section className={`chooser ${yourTurn ? "ready" : "waiting"}`}><p>{yourTurn ? "It is your turn. Select a trait to decide this comparison." : selector.controller === "npc" ? `${nameForPlayer(room, selector.id)} is considering which trait to select…` : `${nameForPlayer(room, selector.id)} is taking a turn.`}</p><div>{STATS.map((stat) => <button key={stat} disabled={!yourTurn || busy} onClick={() => onAct({ action: "choose", stat })}><span>{stat === "strength" ? "⚔" : stat === "zeal" ? "✦" : "◆"}</span>{stat}</button>)}</div></section> : winner && <section className="victory"><EparchCrownMark /><p className="kicker">VICTORY</p><h2>{winner.id === yourSeat.userId ? "You are victorious" : `${nameForPlayer(room, winner.id)} is victorious`}</h2><div className="victoryActions"><button onClick={onLeave}>New Multiplayer Game</button><a className="buttonLink secondary" href="/beginner">Beginner Menu</a></div></section>}{error && <p className="formError multiplayerError" role="alert">{error}</p>}</main>;
}
