...
export default function MasterClient() {
  ...
  if (screen === "home") return <main className="landing masterLanding"><section className="panel titlePanel">
    ...
    <div className="routeLinks"><a href="/">All Game Levels</a><a href="/beginner">Beginner Game</a><a href="/amateur">Amateur Game</a><button className="textButton" onClick={() => setHelp(true)}>Master Rules</button></div>
  ...
  return <main className="gamePage amateurGame masterGame">
    ...
    {!review && humanTurn && <section className="chooser amateurChooser"><p>{!selectedStat ? "Which statistic will decide the attack?" : !attackerId ? "Now choose one of your piles—or your heir—to attack." : "Now select an enemy pile. An exposed heir may also be selected."}</p><div>{STATS.map((stat) => <button key={stat} className={selectedStat === stat ? "chosenStat" : ""} onClick={() => { setSelectedStat(stat); setAttackerId(undefined); }}><span>{stat === "strength" ? "⚔" : stat === "zeal" ? "✦" : "◆"}</span>{stat}</button>)}</div></section>}
