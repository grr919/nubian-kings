...
export function chooseNpcStat(profile:StatProfile,visibleCard:StatProfile|undefined,random:()=>number,available:readonly Stat[]=stats):Stat{if(!available.length)throw new Error("At least one statistic must be available");const weights=visibleCard?Object.fromEntries(stats.map(s=>[s,Math.max(1,visibleCard[s]**2)]))as StatProfile:profile;const total=available.reduce((n,s)=>n+Math.max(0,weights[s]),0);if(total<=0)return available[Math.floor(random()*available.length)];let roll=random()*total;for(const s of available){roll-=Math.max(0,weights[s]);if(roll<0)return s}return available[available.length-1]}
